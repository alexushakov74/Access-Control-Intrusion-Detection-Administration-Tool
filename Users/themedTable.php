...
?>
				...
				data-target="#add" data-id="<?= $row[0] ?>" id="addRow"><strong><span class="glyphicon glyphicon-plus"></span> Add User</strong></button>
				<br>
...
{ 
$emailArray = explode("@", $row[9]);	//[0] is name (bittarn1), [1] is ending part (@owls.southernct.edu)
?>
	<tr>
		...
		<td><?= $row[7] ?></td>
		<td><?= $row[8] ?></td>
		<td><a onClick="javascript:window.open('mailto:<?= $row[9] ?>', 'mail');event.preventDefault()" href="mailto:<?= $row[9] ?>"><?= $emailArray[0] ?></a></td>
...
<script type="text/javascript">
	...
	{ 
		...
		// Call the web page that will get the list of
		$("#modal-content-add").load("addUser.php", function(response, status, xhr) 
		{
