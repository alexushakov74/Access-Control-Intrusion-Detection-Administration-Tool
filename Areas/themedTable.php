...
type="button" class="btn btn-info btn-s" id="relTable" ><span class="glyphicon glyphicon-link"></span></a>
				...
				data-target="#add" data-id="<?= $row[0] ?>" id="addRow"><strong><span class="glyphicon glyphicon-plus"></span> Add Area</strong></button>
				<br>
...
$buildingID = $row[2];
$buildingName;
$sql3 = "SELECT (`Name`) FROM Buildings WHERE `ID`=$buildingID";
...
?>
	<tr>
		...
		<td><?= $row[1] ?></td>
		<td><?php echo $row[2] . ": " . $buildingName ?></td>
...
<div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
				<h4 class="modal-title" id="myModalLabel">Add Area Form</h4>
			</div>
			<div class="modal-body">
...
<script type="text/javascript">
	...
	{ 
		...
		// Call the web page that will get the list of
		$("#modal-content-add").load("addArea.php", function(response, status, xhr) 
		{
